...
#read original data
#huricanes: 161 years (1851-2011)
orig.data = read.csv("Hurricanes.csv", sep=";" ,header=TRUE)

data_mtx           = orig.data
n				   = dim(data_mtx)[1]
rownames(data_mtx) = c(1:n)
...
for (row in 2: n){
	if (data_mtx[row-1,6]==12  & data_mtx[row,6]==1){
		data_mtx[row,5]=data_mtx[row-1,5]+1
	}
	if (data_mtx[row,6]==1  & data_mtx[row-1,6]==1){
		data_mtx[row,5]=data_mtx[row-1,5]
	}
}
...
colnames(data_vec) = c("wind", "year", "month", "day", "6hour_period", 
"year_order", "day_order", "time_order", "day_order_py","time_order_py")
for (row in 1: n){
	for (hrs in 1: 4){
...
# compute expetile curves
# output: "expect_list" - object, a list with length 65 (every year 1946-2010)
#		  of  matrices 1460x9  (every 6 hour, 9 tau levels)
...
for (year in 1947: 2011){
	...
	exp		 				 = expectreg.ls(yeardata_new~rb(xos_new,type="pspline"),
					 			smooth="aic",expectile=c(1:9)/10)
	expfit 	 				 = exp$fitted
	expfit[expfit<0] 		 = 0
...
pdf(plot_path)
	plot(yeardata, main="Hurricanes in 2005", xlab="time per year",
		 ylab="wind (in knots)", ylim=c(0,200),
	axes=FALSE, xlim=c(1,1460))
    axis(side = 1, at = c(1,120), labels=c("",""))
    axis(side = 1, at = c(1:12)*120, 
    	labels=c(" "," "," "," "," "," "," "," "," ", " ", " ", " "))
