...
pdf(plot_path, width=12, height=5)
	plot(P_int_as_matrix[,9],type="l", lty=2, lwd=2, col="darkgreen", xlab="", ylab="", main="")
...
pdf(plot_path)
	...
	plot(beta_fun,lty=1, lwd=2,,axes=FALSE,ylim=c(-0.5,0.5), xlab="time", ylab=" ", main=expression(paste(tau,"=0.6")))
	axis(side = 1, at = c(1/120,120/120), labels=c("",""))
    	axis(side = 1, at = c(1:12)*120/120/12, 
    	labels=c(" "," "," "," "," "," "," "," "," ", " ", " ", " "))			
	month_labels=c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec")
	axis(side = 1, at = ((c(1:12)*120)-60)/12/120, tick=FALSE, 		
    	labels=month_labels)
	axis(side = 2)  #, at = c(-5:5)/10, cex.axis=1 )
	#tau=7
 	beta_fun  = Data2fd(argvals=seq(0,1,length=1460),y= beta_mtx[,7],
 		basisobj=basis)
	plot(beta_fun, col="blue", lty=2, lwd=2,,axes=FALSE,ylim=c(-0.5,0.5), xlab="time", ylab=" ", main=expression(paste(tau,"=0.7")))
	axis(side = 1, at = c(1/120,120/120), labels=c("",""))
    	axis(side = 1, at = c(1:12)*120/120/12, 
    	labels=c(" "," "," "," "," "," "," "," "," ", " ", " ", " "))			
	month_labels=c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec")
	...
	axis(side = 2)  #, at = c(-5:5)/10, cex.axis=1 )	
	#tau=8
 	beta_fun  = Data2fd(argvals=seq(0,1,length=1460),y= beta_mtx[,8],
 		basisobj=basis)
	...
	plot(beta_fun, col="darkgreen", lty=6, lwd=2,,axes=FALSE,ylim=c(-0.5,0.5),
		xlab="time", ylab=" ", main=expression(paste(tau,"=0.9")))
	axis(side = 1, at = c(1/120,120/120), labels=c("",""))
    	axis(side = 1, at = c(1:12)*120/120/12, 
    	labels=c(" "," "," "," "," "," "," "," "," ", " ", " ", " "))			
	month_labels=c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec")
...
for (tau_row in 1:9){
	...
	basis 			= create.bspline.basis( c(0,1), nbasis=365,norder=4)
	beta_hat_fun 		= Data2fd(argvals=seq(0,1,length=1460), 
	y			= beta_hat, basisobj=basis)
...
pdf(plot_path)
plot(sqrt(beta_int_tau), cex.main=1.5 , cex=1.5, , xlab="", ylab="", frame=TRUE, axes=FALSE, main="Hurricanes", ylim=c(0,0.2))  
