...
pdf(plot_path, width=12, height=5)
	plot(P_int_as_matrix[,9],type="l", lty=2, lwd=2, col="darkgreen",
		 xlab="", ylab="", main="")
...
pdf(plot_path)
 	par(mfrow=c(4,1))
 	#tau=6
 	beta_fun  = Data2fd(argvals=seq(0,1,length=1460),y= beta_mtx[,6], 
 		basisobj=basis)
	...
	axis(side = 2)  #, at = c(-5:5)/10, cex.axis=1 )
	#tau=9
 	beta_fun  = Data2fd(argvals=seq(0,1,length=1460),y= beta_mtx[,9], 
 		basisobj=basis)
...
pdf(plot_path)
	plot(sqrt(beta_int_tau), cex.main=1.5 , cex=1.5, , xlab="", ylab="",  
	frame=TRUE, axes=FALSE, main="Hurricanes", ylim=c(0,0.2))  
